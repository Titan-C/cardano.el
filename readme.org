...
Warning aside, I believe this is an awesome way to interact with the Cardano
blockchain, it frees you from the cumbersome CLI too by offering you a
declarative way to specify your transactions, without loosing fine grain
control.
...
The easiest way to get one setup for the public testnet is using this repo:
...
;; Mind also using separate directories for mainnet and testnets
...
you control on your wallet. Select one or many, you can still include more into
your transaction during the edit process later on. A new buffer opens with the
basic spending transaction template you can directly edit.
...
outputs:
  ...
  # Payment to a Plutus script. The always succeds script
...
- =cardano-address-helm= Pick from all your addresses, organized by the key name
...
That's why the datum needs to be a tuple and we need to use the typed version
for the datum, because tuples are not available as JSON values.

In this repo you can find the datum file used for this example in the path
...
token: a fungible token with unconstrained minting policy and a NFT policy.
...
# Minting policies are characterize by the policy-id, here you can name them,
...
# need to sign the transaction. This are the keys on your wallet. I commented
...
# For Mary NFTs don't forget to match the validity interval, with the one on the
# time lock policy
validity-interval:
...
Let's
create a simple spending transaction where we start with our funded UTxO and
spend it to fund a Plutus script.
